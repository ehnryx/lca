...
struct capped_int {
  using capped_t = conditional_t<
    (cap_ll < numeric_limits<int>::max()), unsigned int, unsigned long long>;
  static constexpr capped_t cap = (capped_t)cap_ll;
  capped_t v;
  capped_int(): v(0) {}
  capped_int(unsigned int _v): v(min((capped_t)_v, cap)) {}
  capped_int(unsigned long long _v): v((capped_t)min(_v, (unsigned long long)cap)) {}
  capped_int(int _v) {
    if (_v < 0) throw invalid_argument("cannot create a capped_int from a negative number");
    v = min((capped_t)_v, cap);
  }
  capped_int(long long _v) {
    if (_v < 0) throw invalid_argument("cannot create a capped_int from a negative number");
    v = (capped_t)min((unsigned long long)_v, (unsigned long long)cap);
  }
  ...
  bool operator <= (const other_t& o) const { return operator == (o) || operator < (o); }
  template <typename other_t>
  bool operator > (const other_t& o) const { return !operator <= (o); }
  template <typename other_t>
  bool operator >= (const other_t& o) const { return !operator < (o); }
  friend ostream& operator << (ostream& os, const capped_int& out) { return os << out.v; }
};
